...
$.fn.Cart = function (options) {
    ...
    function _addProductRow(product) {
        ...
        $('#item-table>tbody').append(
            ...
            "<img src='/static/img/products/" + imageFile + "' alt='image' class='img-thumbnail rounded mx-auto d-block' style='width:100px;'>" +
            "</td>" +
            ...
            "<td class='text-center'><a href='#' id='remove-product' class='btn btn-primary height-auto btn-sm'>X</a></td>" +
            "</tr>"
        )
    }
    ...
    function _removeItemFromCart(pid) {
        ...
        const index = cart.findIndex(x => String(x.pid) === String(pid));
        if (index >= 0) {
            cart.splice(index, 1);
        }
    ...
    function _subtractItemFromCart(pid, count) {
        ...
        const index = cart.findIndex(x => String(x.pid) === String(pid));
        if (index >= 0) {
            let item = cart.find(x => String(x.pid) === String(pid));
